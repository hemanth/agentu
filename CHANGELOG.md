...
- **Python 3.9 Compatibility**: Lazy-initialize `asyncio.Semaphore` in `TaskQueue` (supersedes incomplete v2.5.5 PyPI upload that was missing this fix).
...
- **MCP STDIO Transport**: Lazy-initialize `_read_lock` in `MCPSTDIOTransport` to avoid `RuntimeError: There is no current event loop` during synchronous instantiation under Python 3.9 & 3.10.

## [2.5.2] - 2026-08-06

### Changed

- **`with_plugins()`** — Promoted to the primary method for Agent Plugins, maintaining consistency with `with_tools()`, `with_skills()`, and `with_agents()`. Accepts single path or list of paths. `with_plugin()` remains as an alias.

## [2.5.1] - 2026-08-06

### Changed

- **`with_plugin()`** — Unified to accept either a single plugin path or a list of plugin paths (`Union[str, Path, List[str]]`), matching the pattern of `with_tools()` and `with_skills()`. `with_plugins()` remains as an alias.
...
- **`with_agents()`** — give an orchestrator agent access to specialist agents as callable tools. Each child agent becomes a tool (`call_{name}`) that the LLM can invoke dynamically, enabling sequential, parallel, conditional, and looping workflows without a graph framework
- 11 new tests for agent delegation
